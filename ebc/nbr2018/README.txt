...
The Capa content type
===============================

In this section we are tesing the Capa content type by performing
basic operations like adding, updadating and deleting Capa content
items.

Adding a new Capa content item
--------------------------------
...
'Capa' and click the 'Add' button to get to the add form.

    >>> browser.getControl('Capa').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'Capa' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'Capa Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'Capa' content item to the portal.

Updating an existing Capa content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New Capa Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New Capa Sample' in browser.contents
    True

Removing a/an Capa content item
--------------------------------

If we go to the home page, we can see a tab with the 'New Capa
...
Now we are going to delete the 'New Capa Sample' object. First we
go to the contents tab and select the 'New Capa Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New Capa Sample').click()
...
So, if we go back to the home page, there is no longer a 'New Capa
Sample' tab.
...
Adding a new Capa content item as contributor
------------------------------------------------

Not only site managers are allowed to add Capa content items, but
...
We select 'Capa' and click the 'Add' button to get to the add form.
...
Done! We added a new Capa content item logged in as contributor.
...
The Cadastro content type
===============================

In this section we are tesing the Cadastro content type by performing
basic operations like adding, updadating and deleting Cadastro content
items.

Adding a new Cadastro content item
--------------------------------
...
Then we select the type of item we want to add. In this case we select
'Cadastro' and click the 'Add' button to get to the add form.

    >>> browser.getControl('Cadastro').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'Cadastro' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'Cadastro Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'Cadastro' content item to the portal.

Updating an existing Cadastro content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New Cadastro Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New Cadastro Sample' in browser.contents
    True

Removing a/an Cadastro content item
--------------------------------

If we go to the home page, we can see a tab with the 'New Cadastro
Sample' title in the global navigation tabs.
...
Now we are going to delete the 'New Cadastro Sample' object. First we
go to the contents tab and select the 'New Cadastro Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New Cadastro Sample').click()
...
So, if we go back to the home page, there is no longer a 'New Cadastro
Sample' tab.
...
Adding a new Cadastro content item as contributor
------------------------------------------------

Not only site managers are allowed to add Cadastro content items, but
also site contributors.
...
We select 'Cadastro' and click the 'Add' button to get to the add form.
...
Done! We added a new Cadastro content item logged in as contributor.
